...
template<typename T>
void getRecurrenceFrac(int k, T const* a, T const* c, T* p, T* q) {
  q[0] = 1;
  for (int i = 0; i < k; ++i)
    q[i + 1] = -c[i];
  if (k == 0) return;
  polyMul(a, k, q, k + 1, p, k);
}

template<typename T>
T getNthRecFast(int k, T const* a, T const* c, ll n) {
  assert(n >= 0);
  if (k == 0) return 0;
  if (n < k) return a[n];
  vector<T> p(k * 2, 0);
  vector<T> q(k * 2 + 2, 0);
  getRecurrenceFrac(k, a, c, p.data(), q.data());
  while (true) {
    T* q2 = q.data() + k + 1;
    for (int i = 0; i <= k; ++i)
      q2[i] = (i & 1 ? -q[i] : q[i]);
    polyMul(p.data(), k, q2, k + 1, p.data(), k * 2);
    polyMul(q.data(), k + 1, q2, k + 1, q.data(), k * 2 + 1);
    for (int i = 0; i <= k; ++i)
      q[i] = q[i * 2];
    if (n & 1) {
      for (int i = 0; i < k; ++i)
...
template<typename T>
T getNthRecFast(vector<T> const& a, vector<T> const& c, ll n) {
  int k = sz(a);
  assert(k == sz(c));
  return getNthRecFast(k, a.data(), c.data(), n);
}
